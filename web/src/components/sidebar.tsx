...
// TODO: интересно, можно ли красивей написать? Без JSX.Element в качестве состояния
// это ведь просто список, который должен перерендериваться, если произошли изменения 
// forEach по списку channel_context, изи же? 
...
function SideBar() {
	...
	return (
		<div className={`h-full w-full lg:w-64 ${location.pathname !== "/channels" ? "hidden" : "block"} overflow-y-scroll lg:block md:border-r border-zinc-800`}>
			<SideBarHeader />
			{/* через forEach нельзя написать, потому что тогда реакт не поймет, надо ли перерендеривать, если что-то изменилось?  */}
			{channels_element}
		</div>
	);
}
